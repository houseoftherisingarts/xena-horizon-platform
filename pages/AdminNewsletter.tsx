  ...
  return (
    <div data-tx-scope="adminInfolettre" className="px-6 md:px-10 py-10 space-y-8">
      ...
      <div className="flex items-center gap-2">
        <button type="button" onClick={() => setOnglet('lettres')} className={pill(onglet === 'lettres')}><FileText className="w-3.5 h-3.5" aria-hidden="true" /> {t.lettres}</button>
        <button type="button" onClick={() => ouvrir(null)} className={pill(false)}><Pen className="w-3.5 h-3.5" aria-hidden="true" /> {t.composer}</button>
        <button type="button" onClick={() => setOnglet('abonnes')} className={pill(onglet === 'abonnes')}><Users className="w-3.5 h-3.5" aria-hidden="true" /> {t.abonnes}</button>
      </div>
