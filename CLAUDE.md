...
### Architecture
- Pas de routeur : `ViewState` dans `App.tsx`, une adresse par vue dans `lib/routes.ts` (`/`, `/services`, `/projets`, `/espace`, `/admin/...`). Le back-office se charge en différé. La langue (`lang`) vit dans `App.tsx`, persiste dans `localStorage` `xena.lang` et pose `<html lang>`; l'interrupteur est `components/BasculeLangue.tsx`.
...
- SEO et GEO (8 sept au soir) : `scripts/prerender-meta.mjs` écrit le vrai contenu dans `#root` pour `/`, `/services`, `/projets`, `/a-propos` (h1, sections, tiré de `lib/contenu.ts`), JSON-LD `@graph`, sitemap, `robots.txt` (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, CCBot nommés; `/admin`, `/espace`, `/history` interdits), `dist/404.html` noindex, `history/v1` refermée; `public/llms.txt`; image `og-1200x630.jpg`; clé IndexNow `public/ec2521ca0d8568feb54ce935a5d660bd.txt` et `scripts/indexnow-ping.mjs` en fin de build. `firebase.json` : plus de rewrite fourre-tout (une adresse inconnue rend un vrai 404), seule `/admin{,/**}` est réécrite vers `index.html`; CSP `script-src` par hachage `sha256-DhBxekCl5C3ciyi5duwDEQ4DzBqpUP91qQ8r6aE7r9o=` (à recalculer si le script inline d'`index.html` change) sans `unsafe-inline` ni `unsafe-eval`, et `www.googletagmanager.com` permis pour Analytics. Le pied de page crédite « Site créé par Vexel Webstudio » en lien vers vexelwebstudio.com.
- Composants portés de 21st.dev (8 sept, tard; code source fourni par Alex, réécrit avec nos tokens, sans shadcn ni Radix) : `components/motion/MotsTournants.tsx` (Animated hero de tommyjepsen : « Pour les [public] » sous le sous-titre du hero, ressort stiffness 50), `components/motion/CarteProjecteur.tsx` (Spotlight card de jahed : fond 200 px, bordure 150 px et reflet blanc 100 px qui suivent le pointeur, un seul écouteur, souris fine seulement; cartes de /services), `components/Faq.tsx` (FAQ 06 de hirael : badge, titre révélé mot à mot en deux tons, lede, cartes qui montent, plusieurs ouvertes; pleine largeur à gauche; six réponses tirées du contenu réel), `components/motion/TexteGluant.tsx` (GooeyText : deux calques flou + seuil alpha SVG, boucle rAF nettoyée; les noms de `CLIENTS_CONFIANCE` se fondent l'un dans l'autre dans `pages/accueil/Clients.tsx`, liste complète dessous), `components/Interrupteur.tsx` (ThemeToggle : pilule 64 × 32, bouton qui glisse de 32 px) qui sert à `BasculePalette.tsx` (nuage = ciel, plume = encre) et à `BasculeLangue.tsx` (FR/EN, remplace l'ancien « FR / EN » texte dans `Nav.tsx`, barre et tiroir).
- Photos recadrables par Laurie (8 sept, tard) : `lib/cadrages.tsx` (`useCadrage(id, positionDeBase)` rend `data-cadre`, `data-cadre-base` et `style` : object-position pour le point focal, propriété CSS `scale` avec `transform-origin` sur le point focal pour le zoom; surcharges dans Firestore `settings/cadrages` `{ id: { x, y, z } }`, cache `xena.cadrages`). Onze photos publiques portent un identifiant (`accueil_hero`, `accueil_apropos`, `accueil_contact`, `accueil_projet_<id>`, `projets_balado`, `projets_modele`, `apropos_hero`, `apropos_scene`, `apropos_evenement`, `services_bande`); `KenBurns` prend `cadre=`. Le crayon (`components/Editeur.tsx`) ouvre sur une photo une fenêtre « Recadrer la photo » (point à glisser, zoom 1 à 3, « Photo de base »), et son « Enregistrer » écrit textes et cadrages ensemble. Toute photo `object-cover` recadrable doit vivre dans un parent `overflow-hidden`. Boucle locale sans compte admin : `VITE_EDITEUR_DEMO=1 npx vite build --mode verif --outDir dist-verif` affiche le crayon (le mode `verif` est exigé en plus de la variable; un build de production ne le compile jamais).
- Mode nuit (8 sept, tard) : deuxième dimension indépendante de la palette. `data-nuit` sur `<html>` (lib/skin.ts `useNuit`, clé `xena.nuit`, sinon `prefers-color-scheme`), blocs `:root[data-nuit]` et `:root[data-nuit][data-skin='encre']` dans `index.css` (papier et encre renversés, accent gardé, ombres noires); la classe `.encre` des feuilles sombres se re-renverse la nuit pour rester sombre d'un ton. Interrupteur `components/BasculeNuit.tsx` (soleil/lune) dans la barre et le tiroir. Le script inline d'`index.html` pose aussi cet attribut : hash CSP `sha256-DhBxekCl5C3ciyi5duwDEQ4DzBqpUP91qQ8r6aE7r9o=`.
- Mouvement : `lib/motion.ts` force les animations partout (`useReducedMotion` rend toujours faux); le site n'obéit plus au réglage système « réduire les animations » depuis le 8 sept (Laurie ne voyait rien sur son poste).
- Coffre « Pour Vexel » : depuis le 8 sept au soir, le formulaire demande aussi le nom légal, le NEQ, les numéros de TPS et de TVQ, et les accès Stripe et Google (courriel et mot de passe, chiffrés comme le reste); `scripts/coffre-lire.mjs` masque les mots de passe sans `--complet`.
- Salle de presse (23 sept. 2026) : `/presse` (`pages/PublicPresse.tsx`) montre cinq cartes 1920 × 1080 rendues en direct par `components/presse/CartePresse.tsx`, la grille des photographies pleine résolution et trois textes. Le contenu vit dans `lib/presse-contenu.ts` (types, `KIT_DEFAUT` tiré de `lib/contenu.ts`, hook `usePresse`), publié dans Firestore `settings/presse` sous la forme d'un seul champ texte `json`; le brouillon de Laurie vit dans `brouillons/presse` (admin seulement, règle ajoutée). Le bouton Télécharger capture le composant hors écran par `html-to-image` : un seul gabarit, donc l'aperçu et le fichier livré ne peuvent pas diverger. Le zip d'un seul geste se fabrique au clic (`lib/zip.ts`, méthode « stored », test `node scripts/zip.test.mjs`). L'admin est `pages/AdminPresse.tsx` (Admin › Salle de presse) : textes bilingues, choix ou téléversement de photo vers Storage `site/presse/`, cadrage au doigt, brouillon au fil de la frappe puis « Publier les changements ». La mise en page est une manchette (texte sur le papier, photo dans son panneau) et non une affiche à bandeau : les photos de Laurie sont en portrait, un cadre 16:9 plein écran n'en garderait qu'une bande. Les couleurs de la carte sont écrites en dur, pour qu'un fichier livré ne change pas selon la palette ni le mode nuit du visiteur. `/presse` est prérendu et indexé (`scripts/prerender-meta.mjs`, `contenuPresse()`), et le code QR `public/presse/qr-presse.png` pointe sur la page. Boucle visuelle : `node scripts/qa-presse.cjs http://localhost:4173 tmp/presse-shots`.
...
node scripts/qa-21st.cjs http://localhost:4173 captures-21st # mots tournants, interrupteurs (ciel/encre, FR/EN), cartes, FAQ, tiroir mobile
npx vite build --outDir dist-verif && npx vite preview --outDir dist-verif --port 4173 --strictPort   # bâtir et servir à côté de dist/ quand un agent bâtit en même temps
