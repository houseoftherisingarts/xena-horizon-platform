...
- Prise de rendez-vous (`pages/accueil/Contact.tsx`) : écrit le lead, crée le compte Firebase avec le même courriel (mot de passe aléatoire), ouvre `dossiers/{uid}` avec le message, envoie le courriel de mot de passe (`sendPasswordResetEmail`). Firebase Auth : gabarits en français, expéditeur « Laurie Belhumeur · Xena Horizon », réponse à laurie.belhumeur@gmail.com; le corps du courriel reste celui de Firebase (Identity Platform requis pour le personnaliser).
...
node scripts/qa-v2.cjs http://localhost:4173 captures-v2   # boucle verdict (palettes, éditeur, rendez-vous, espace, admin; comptes témoins dans ~/.config/xena/compte-temoin.txt, uid admin témoin à poser le temps du test dans lib/admins.ts + les règles)
```
