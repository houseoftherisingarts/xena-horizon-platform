...
- Espace client façon profil (Krystine) : `components/espace/EspaceShell.tsx` (bannière `banniereURL` sinon `/images/banniere-defaut.jpg`, avatar `photoURL`, onglets dossier, pieces, parcours, rendezvous, messages, ressources, profil), `MonProfil.tsx` (photo et bannière vers Storage `profils/{uid}/`, bio ≤ 1000, liens ≤ 6, champs permis dans firestore.rules), `Avatar.tsx`.
...
node scripts/qa-rdv.cjs http://localhost:4173 captures-v3  # rendez-vous de bout en bout (demande, confirmation admin, .ics)
```
